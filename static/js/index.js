...
fetch('/api/featured-products/')
  .then(res => res.json())
  .then(dataShortList => {
    const container = document.getElementById("productListShort");
    if (!container) {
      console.error("Контейнер для товаров не найден!");
      return;
    }

    dataShortList.forEach(product => {
      const card = document.createElement("div");
      card.className = "productCardShort";

      card.innerHTML = `
        <img src="${product.image}" alt="${product.title}" class="productImage" />
        <h3 class="productName" style="font-size: 20px">${product.title}</h3>
        <p class="productPrice"><strong>${product.price} €</strong></p>
